...
class Store {
      ...
      rewardPools: [
          ...
          tokens: [
            {
              ...
              abi: config.erc20ABI,
              decimals: 18,
              rewardsAddress: config.daiRewardsAddress,
              rewardsABI: config.daiRewardsABI,
              rewardsSymbol: 'YFU',
              decimals: 18,
              balance: 0,
              stakedBalance: 0,
        ...
        },
        {
          ...
          tokens: [
            {
              id: 'UNI',
              address: '0xa9e98dc206f21fa0992ec640ba37ab4f62b223e9',//CHANGE
              symbol: 'UNI-LP',
              abi: config.erc20ABI, //CHANGE
              booster: '0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984',//CHANGE
              boosterSymbol: 'UNI',
              decimals: 18,
              rewardsAddress: config.uniethRewardsAddress, //CHANGE
              rewardsABI: config.uniethRewardsABI, //CHANGE
              rewardsSymbol: 'YFU', //CHANGE?
              decimals: 18,
              balance: 0,
              stakedBalance: 0,
              rewardsAvailable: 0,
      ...
      function (payload) {
        switch (payload.type) {
          ...
          case GET_YCRV_REQUIREMENTS:
            ...
            break;
          case GET_BOOST_INFO:
            this.getBoosterInfo(payload)
            break;
          // case GET_BOOSTERS:
          //   this.getBoosters(payload)
          //   break;
          // case GET_BOOSTER_COST:
          //   this.getBoosterCost(payload)
          //   break;
          case BOOST:
            this.boost(payload)
            break;
    ...
    async.map(pools, (pool, callback) => {
      ...
      async.map(pool.tokens, (token, callbackInner) => {
        ...
        async.parallel([
          ...
          token.boosters = data[0]
          token.boosterPrice = data[1]
      ...
      store.setStore({rewardPools: poolData})
      emitter.emit(GET_BOOST_INFO_RETURNED)
    })
  }
  ...
  boost = async (payload) => {
    const account = store.getStore('account')
    const { asset, amount } = payload.content//const { asset } = payload.content

    this._checkBoosterApproval(asset, account, amount, asset.rewardsAddress, (err) => {
      if(err) {
        return emitter.emit(ERROR, err);
      }

      this._callBoost(asset, account, (err, res) => {
        if(err) {
          return emitter.emit(ERROR, err);
        }

        return emitter.emit(BOOST_RETURNED, res)
      })
    })

  }
  _callBoost = async (asset, account, callback) => {
    const web3 = new Web3(store.getStore('web3context').library.provider);
    ...
    boostContract.methods.boost().send({ from: account.address, gasPrice: web3.utils.toWei(await this._getGasPrice(), 'gwei') })
      ...
      .on('confirmation', function(confirmationNumber, receipt){
        ...
        if(confirmationNumber == 2) {
          dispatcher.dispatch({ type: GET_BOOST_INFO, content: {} })
        }
      })
  ...
  _checkBoosterApproval = async (asset, account, amount, contract, callback) => {
    try {
      const web3 = new Web3(store.getStore('web3context').library.provider);

      const erc20Contract = new web3.eth.Contract(asset.abi, asset.booster)
  ...
  _checkBoosterApprovalWaitForConfirmation = async (asset, account, amount, contract, callback) => {
    const web3 = new Web3(store.getStore('web3context').library.provider);
    let erc20Contract = new web3.eth.Contract(config.erc20ABI, asset.booster)
    const allowance = await erc20Contract.methods.allowance(account.address, contract).call({ from: account.address })
