...
import girlOne from '../../assets/yfu/tenor.gif'
import '../../assets/yfu/bubbles.css'
import {
  ERROR,
  CONFIGURE_RETURNED,
  GET_BALANCES,
...
const messageOne = {
    background: 'url('+girlOne+') no-repeat center bottom',
    position: "absolute",
    bottom: 0,
    left:0,
    width: 220,
...
class RewardPools extends Component {
  ...
  componentWillMount() {
    ...
    emitter.on(GET_BOOST_INFO_RETURNED, this.boostInfoReturned);
  ...
  componentWillUnmount() {
    ...
    emitter.removeListener(GET_BOOST_INFO_RETURNED, this.boostInfoReturned);
  ...
  render() {
    ...
    return (
      <div className={ classes.root }>
        <div style={messageOne}>
          <div className={'speech-bubble'}>
            So many YFU pools! So little time!
          </div>
          <br />
        </div>
  ...
  renderRewards = () => {
    ...
    return rewardPools.filter((rewardPool) => {
      if(['GovernanceV2', 'Seed', 'BoostUNI'].includes(rewardPool.id) ) {
        return true
      }
    }).map((rewardPool) => {
